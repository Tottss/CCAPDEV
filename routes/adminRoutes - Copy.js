...
router.post('/api/admin/reserve', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const seat = slot.seats.find(s => s.seatNumber === seatNumber);
    ...
    if (!slot.reservedSeats.includes(seatNumber)) {
      slot.reservedSeats.push(seatNumber);
    }

    await roomDoc.save();

    res.status(200).json({ message: `Seat ${seatNumber} reserved successfully.` });
  } catch (err) {
    await logError(err, 'POST /api/admin/reserve');
    console.error(err);
